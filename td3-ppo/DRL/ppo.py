...
class PPO(object):
	def __init__(self, batch_size=64, env_batch=1, max_step=40, \
		...
		self.action_prob = [None] * self.env_batch
		self.choose_device()        
	...
	def play(self, state, target=False):
		...
		if target:
			x = self.actor_target(state)
		else:
			x = self.actor(state)
		#print ("original return shape:", x.shape)
		prob = F.softmax(x)
		m = Categorical(prob)
		#print (m)
		#print (m.shape)
		a = m.sample(sample_shape=torch.Size([65]))
		a = a.view(a.size()[1], a.size()[0])
		a = a.float()
		#print (a)
		#print ("sampled: ", a.shape)
	...
	def update_policy(self, lr):
		...
		# Sample batch
		state, action, reward, action_probs, \
	...
	def observe(self, reward, state, done, step):
		...
		ap = torch.tensor(self.action_prob, device="cpu")
		s1 = torch.tensor(state, device='cpu')
		d = to_tensor(done.astype('float32'), "cpu")
		for i in range(self.env_batch):
			self.memory.append([s0[i], a[i], r[i], ap[i], s1[i], d[i]])
		self.state = state
