...
class ResNet_wobn(nn.Module):
    def __init__(self, num_inputs, depth, num_outputs):
        ...
        self.in_planes = 64
        self.conv2 = conv3x3(num_inputs, 64, 2)
        self.layer5 = self._make_layer(block, 64, num_blocks[0], stride=2)
        self.layer6 = self._make_layer(block, 128, num_blocks[1], stride=2)
        self.layer7 = self._make_layer(block, 256, num_blocks[2], stride=2)
        self.layer8 = self._make_layer(block, 512, num_blocks[3], stride=2)
    ...
    def forward(self, x):
        x1 = self.relu_1(self.conv1(x))
        x1 = self.layer1(x1)
        x1 = self.layer2(x1)
        x1 = self.layer3(x1)
        x1 = self.layer4(x1)
        x1 = F.avg_pool2d(x1, 4)
        x1 = x1.view(x1.size(0), -1)
        x1 = self.fc1(x1)

        x2 = self.relu_2(self.conv2(x))
        x2 = self.layer5(x2)
        x2 = self.layer6(x2)
        x2 = self.layer7(x2)
        x2 = self.layer8(x2)
